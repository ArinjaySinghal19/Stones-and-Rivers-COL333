...
public:
    ...
    Move choose(const std::vector<std::vector<std::map<std::string, std::string>>>& board, 
        ...
        const int MINIMAX_DEPTH = 2;  // BOT2: Depth 2
        std::cout << "[BOT2 - DEPTH=2] Making move for " << side << std::endl;
        ...
        std::cout << "[BOT2] Move returned in " << minimax_duration.count() / 1e6 << " seconds." << std::endl;
...
// ---- PyBind11 bindings ----
namespace bot2 {
    using ::StudentAgent;  // Import from global namespace
}

PYBIND11_MODULE(bot2_module, m) {
    // Note: Move class not exposed to Python - not needed
    
    py::class_<bot2::StudentAgent>(m, "AgentBot2")
        .def(py::init<std::string>())
        .def("choose", &bot2::StudentAgent::choose);
}