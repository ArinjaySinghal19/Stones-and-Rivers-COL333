...
public:
    ...
    Move choose(const std::vector<std::vector<std::map<std::string, std::string>>>& board, 
        ...
        const int MINIMAX_DEPTH = 1;  // BOT1: Depth 1
        std::cout << "[BOT1 - DEPTH=1] Making move for " << side << std::endl;
        ...
        std::cout << "[BOT1] Move returned in " << minimax_duration.count() / 1e6 << " seconds." << std::endl;
...
PYBIND11_MODULE(bot1_module, m) {
    // Note: Move class not exposed to Python - not needed
    
    py::class_<bot1::StudentAgent>(m, "AgentBot1")
        .def(py::init<std::string>())
        .def("choose", &bot1::StudentAgent::choose);
}